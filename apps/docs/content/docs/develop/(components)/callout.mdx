...
If the callout is dismissible, you can display a **close button** on the top right corner of the callout by declaring the `dismissible` prop.
To capture the dismiss event, you may use the `onDismiss` prop.

To dismiss the callout programmatically through any of the event listeners inside `CalloutAction`'s children, you may use the `dismiss` function passed as an argument to the render prop.
...
<Tabs items={["Preview", "Code"]} defaultValue="Preview">
  <Tab value="Preview">
    <div className="not-prose space-y-2">
      ...
      <DismissibleCalloutExample
        type="action"
        title="Action - Dismiss Callout via CalloutAction Render Prop"
        description="You can invoke the dismiss event programmatically through any of event listeners inside CalloutAction's children"
      />
    </div>
  </Tab>
  <Tab value="Code">
...
<TypeTable
  type={{
    children: {
      type: "ReactNode | ((dimiss) => ReactNode)",
    },
  }}
/>
